import { OrganizationPerson, HierarchyFilterOptions, SurveyResponse } from '../types/OrganizationTypes';
...
export class OrganizationService {
  ...
  public static buildHierarchyWithPeople(
    ...
    if (dataSet.paging?.hasNextPage) {
      console.warn('UWAGA: Dataset ma więcej stron danych. Tylko pierwsza strona jest załadowana.');
    }
    ...
    if (filterOptions?.currentUserId) {
      ...
      if (filterOptions.showOnlyTeam !== false) {
        const filteredHierarchy = this.filterByUserId(people, hierarchy, filterOptions.currentUserId);
   ...
   */
  private static buildTreeStructure(people: OrganizationPerson[]): OrganizationPerson[] {
  ...
  private static filterByUserId(
    ...
    if (!currentUser) {
      ...
      currentUser = allPeople.find(person => {
        ...
        const cleanAgUserId = person.ag_userid.replace(/[{}-]/g, '').toLowerCase();
    ...
    if (!currentUser) {
      console.warn(`OrganizationService: Nie znaleziono użytkownika o ID: ${userId}`);
  ...
  public static isPersonInCurrentUserTeam(
    ...
    if (person.ag_userid && currentUserId) {
      ...
      if (person.ag_userid === currentUserId) {
        ...
        const normalizedPersonId = person.ag_userid.replace(/[{}-]/g, '').toLowerCase();
        const normalizedCurrentId = currentUserId.replace(/[{}-]/g, '').toLowerCase();
    ...
    const isTeamMember = this.isPersonSubordinateOf(allPeople, person.id, currentUser.id);
   ...
   */
  private static isPersonInTeam(manager: OrganizationPerson, personId: string): boolean {
   ...
   */
  public static flattenHierarchy(hierarchy: OrganizationPerson[]): OrganizationPerson[] {
  ...
  private static buildSubordinatesHierarchy(
    ...
    const directSubordinates = allPeople.filter(person => person.managerId === managerId);
    
    // Dla każdego podwładnego, rekursywnie zbuduj jego hierarchię
    return directSubordinates.map(subordinate => ({
      ...subordinate,
      children: this.buildSubordinatesHierarchy(allPeople, subordinate.id, level + 1)
    }));
  }
