...
export const OrganizationTree: React.FC<OrganizationTreeProps> = ({
  ...
  const handleSurveyClick = React.useCallback((personId: string) => {
  ...
  const handleResponseClick = React.useCallback((responseUrl: string) => {
    if (onResponseClick) {
  ...
  const buildLayout = React.useCallback(() => {
    ...
    const { hierarchy: fullOrganizationHierarchy, allPeople: allPeopleData } = OrganizationService.buildHierarchyWithPeople(dataSet);
    ...
    const { hierarchy: organizationHierarchy } = OrganizationService.buildHierarchyWithPeople(dataSet, filterOptions);
    setHierarchy(organizationHierarchy);

    const { nodes: layoutNodes, edges: layoutEdges } = LayoutService.createTreeLayout(
  ...
  }, [dataSet, surveyUrl, userId, showOnlyTeam, surveyResponses, handleSurveyClick, handleResponseClick, setNodes, setEdges]);

  // Efekt do odbudowy layoutu przy zmianie danych
  React.useEffect(() => {
    if (dataSet?.sortedRecordIds?.length && dataSet.sortedRecordIds.length > 0) {
      buildLayout();
    }
  }, [buildLayout]);
  ...
  React.useEffect(() => {
    if (surveyResponsesDataSet?.sortedRecordIds?.length && surveyResponsesDataSet.sortedRecordIds.length > 0) {
      const responses = OrganizationService.processSurveyResponses(surveyResponsesDataSet, surveyId);
      ...
      <div className={styles.container}>
        <div className={styles.reactFlowWrapper}>
          ...
          >
            ...
            <Panel 
              position="top-left" className={styles.panel}
                style={{ 
                top: 180,
                left: -14,
